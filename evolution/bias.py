...
def monitor_prediction_diversity(population, X, y, step, debug=False):
    ...
    def _run_diversity_check(population, X, y, step):
        """Internal function to run the diversity check"""
        if step % 3 == 0:
    ...
    if not debug:
        with suppress_print():
            _run_diversity_check(population, X, y, step)  
    else:
        _run_diversity_check(population, X, y, step)
...
def monitor_prediction_diversity_with_action(population, X, y, step, max_steps=75, bias_threshold=None, debug=False):  # LOWERED from 0.85
    ...
    def _run_diversity_check(population, X, y, step, max_steps, bias_threshold):
        """Internal function to run the diversity check"""
        bias_report = {
            'step': step,
            'biased_modules': [],
    ...
    if not debug:
        with suppress_print():
            return _run_diversity_check(population, X, y, step, max_steps, bias_threshold)
    else:
        return _run_diversity_check(population, X, y, step, max_steps, bias_threshold)
...
def catalyst_bias_elimination(population, bias_report, elimination_rate=0.25, debug=False):
    ...
    def _run_catalyst_elimination(population, bias_report, elimination_rate):
        """Internal function to run the catalyst-based elimination"""
    ...
    if not debug:
        with suppress_print():
            return _run_catalyst_elimination(population, bias_report, elimination_rate)
    else:
        return _run_catalyst_elimination(population, bias_report, elimination_rate)
